...
- **Link order (Mandatory):** Maps, Site, Photo, Phone. Phone link uses `<a href="tel:{number}" class="poi-card__link">` with the phone SVG icon + `📞` emoji + localized label. If no phone number exists in the markdown source, no phone link is rendered (no empty/placeholder link). The `href` value strips all characters except digits and leading `+` from the phone number (extensions are dropped).
